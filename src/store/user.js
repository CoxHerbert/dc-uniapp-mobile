...
function readLoginInfo() {
  try {
    const stored = storage.get(KEYS.LOGIN_INFO, null);
  ...
  actions: {
    ...
    setLoginInfo(info) {
      if (isObject(info)) {
        this.loginInfo = info;
        try {
          storage.set(KEYS.LOGIN_INFO, JSON.stringify(info));
        } catch (err) {
          console.warn('[user-store] failed to persist login info:', err);
          storage.set(KEYS.LOGIN_INFO, String(info));
        }
      } else {
        this.loginInfo = null;
        try {
          storage.remove(KEYS.LOGIN_INFO);
        } catch (err) {
